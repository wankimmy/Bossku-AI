...
- **Recovered harness edits in the agents.** An earlier harness session's agent improvements survived only in the installed `~/.claude/agents` copies. They are merged back: the auditor runs cheap verification itself and quotes the real output, the executor reads `DESIGN.md` before UI edits and names `bosskuai-ponytail` + `bosskuai-rigorous-code-review` for the de-sloppify pass, the planner gets read-only Bash, and the orchestrator lists every specialist role as agent + skill, claims work in `.bossku/memory/handoff.md`, and runs loops with exit conditions named up front. The UI screenshot gates stay out until `agents/designer.md` and `scripts/shot.mjs` are committed.
