---
name: graft
description: "Use when the repo has a graft/ context index built by the graft CLI. For ANY task in that repo, whether understanding how something works, finding where code lives, tracing what calls a symbol or what a change breaks, or scoping an edit, get context from graft before grepping or reading source files. No graft/ directory means no index: build one with `graft build` or work normally."
---
