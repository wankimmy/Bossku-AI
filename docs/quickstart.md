...
**If Stream run fails immediately or `/api/runs` returns 500:** check `docker compose logs -f backend` for `bootstrap/cache` / `storage` not writable. Rebuild the backend image (`docker compose build backend && docker compose up -d backend`); the Dockerfile entrypoint fixes permissions on container start. Validate with `curl -i http://localhost:28480/api/runs`.
