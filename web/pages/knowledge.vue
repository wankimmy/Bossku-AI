...
<template>
  <div class="mx-auto flex min-w-0 w-full max-w-7xl flex-col gap-6 overflow-hidden">
    <section class="min-w-0 rounded-3xl border border-emerald-400/20 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.18),transparent_32%),linear-gradient(135deg,#0f172a,#09090b_58%,#111827)] p-5 shadow-2xl shadow-black/30 sm:p-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p class="mt-2 max-w-2xl text-sm text-zinc-400">
            Paste articles, Google results, YouTube links, or import local Codex and Claude memory. Bossku stores the useful text as active searchable knowledge.
          </p>
        </div>
        <button
          type="button"
          class="rounded-xl border border-zinc-700 px-4 py-2 text-sm text-zinc-300 hover:border-emerald-400 hover:text-emerald-200"
          @click="refresh()"
        >
          Refresh
        </button>
