...
async function loadRoot() {
  try {
    const root = await api.get<{ root: string; relative: string }>('/project')
  ...
  <div class="space-y-4">
    <header class="rounded-lg border border-zinc-800 bg-zinc-900 p-4">
      ...
      <p class="mt-1 text-sm text-zinc-400">
        Browse and edit files under the Docker repo mount
        <code class="rounded bg-zinc-950 px-1 text-xs text-emerald-400">{{ repoRoot || '/repo' }}</code>.
        Writes require approval before they are applied.
      </p>
    </header>
