...
export function useEditorActions(
  ...
  const saveLayout = useCallback((layout: OfficeLayout) => {
    ...
    saveTimerRef.current = setTimeout(() => {
      bossku.postMessage({ type: 'saveLayout', layout })
  ...
  const handleOpenClaude = useCallback(() => {
    bossku.postMessage({ type: 'openClaude' })
  }, [])
  ...
  const handleEditorTileAction = useCallback((col: number, row: number) => {
    ...
    if (editorState.activeTool === EditTool.TILE_PAINT) {
      ...
      const hit = findTopmostFurnitureAt(layout, col, row)
      if (hit) {
      ...
      if (type === '') {
        ...
        const { col: placeCol, row: placeRow } = resolveFurniturePlacement(layout, type, col, row)
        if (!canPlaceFurniture(layout, type, placeCol, placeRow)) return
        const uid = `f-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`
        const placed: PlacedFurniture = { uid, type, col: placeCol, row: placeRow }
