...
/** Find desk top-left if (col, row) is on a desk tile. */
export function findDeskAt(
  layout: OfficeLayout,
  col: number,
  row: number,
...
/** Topmost furniture at tile (last in paint order). */
export function findTopmostFurnitureAt(
  layout: OfficeLayout,
  col: number,
  row: number,
...
/** Snap surface items to desk origin; wall items to wall row. */
export function resolveFurniturePlacement(
  ...
  if (!entry) return { col, row }
  if (entry.canPlaceOnSurfaces) {
    const desk = findDeskAt(layout, col, row)
    if (desk) return desk
  }
  if (entry.canPlaceOnWalls) {
    return { col, row: getWallPlacementRow(type, row) }
  }
  return { col, row }
}
...
export function canPlaceFurniture(
  ...
  // Surface items must sit on at least one desk tile; other footprint cells must be clear floor
  if (entry.canPlaceOnSurfaces) {
    if (!deskTiles || deskTiles.size === 0) return false
    let onDesk = false
    ...
    for (let dr = 0; dr < entry.footprintH; dr++) {
      ...
      for (let dc = 0; dc < entry.footprintW; dc++) {
        ...
        const idx = (row + dr) * layout.cols + (col + dc)
        const tileVal = layout.tiles[idx]
        if (tileVal === TileType.VOID || tileVal === TileType.WALL) return false
      }
    }
