...
let defaultLaptopType: string | null = null
...
export function buildDynamicCatalog(assets: LoadedAssetData): boolean {
  ...
  // Phase 4: synthetic rotation groups for single-orientation assets
  const syntheticOrients = ['front', 'right', 'back', 'left'] as const
  for (const entry of allEntries) {
    if (rotationGroups.has(entry.type)) continue
    if (entry.canPlaceOnWalls) continue
    const members: Record<string, string> = {}
    for (const o of syntheticOrients) {
      members[o] = entry.type
    }
    const rg: RotationGroup = { orientations: [...syntheticOrients], members }
    rotationGroups.set(entry.type, rg)
  }

  defaultLaptopType = null
  for (const asset of assets.catalog) {
    if (asset.groupId !== 'LAPTOP') continue
    if (asset.state && asset.state !== 'off') continue
    if (asset.orientation && asset.orientation !== 'front') continue
    defaultLaptopType = asset.id
    break
  }
  if (!defaultLaptopType) {
    for (const asset of assets.catalog) {
      if (!asset.canPlaceOnSurfaces) continue
      if (asset.state && asset.state !== 'off') continue
      if (asset.orientation && asset.orientation !== 'front') continue
      defaultLaptopType = asset.id
      break
...
/** Returns true if the given furniture type can be rotated in the editor. */
export function isRotatable(type: string): boolean {
  const entry = getCatalogEntry(type)
  if (!entry) return false
  if (entry.canPlaceOnWalls) return false
  return rotationGroups.has(type)
}

/** Default laptop asset id (front/off) for Tech toolbar, or null if catalog has none. */
export function getDefaultLaptopType(): string | null {
  return defaultLaptopType
}
