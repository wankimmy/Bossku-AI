...
final class ToolCallFormatter
{
    ...
    public static function actionDetail(string $tool, array $payload): string
    {
        return match ($tool) {
            ...
            'mcp_list_tools' => 'Listing MCP tools: '.self::string($payload['server'] ?? '(server missing)'),
            'mcp_call' => 'MCP '.self::string($payload['server'] ?? '?').'.'.self::string($payload['tool'] ?? '?'),
    ...
    public static function payloadPreview(string $tool, array $payload): array
    {
        return match ($tool) {
            ...
            'mcp_list_tools' => ['server' => self::string($payload['server'] ?? '')],
            'mcp_call' => ['server' => self::string($payload['server'] ?? ''), 'tool' => self::string($payload['tool'] ?? '')],
