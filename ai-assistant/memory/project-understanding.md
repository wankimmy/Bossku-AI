...
- Current expert surface size: Confirmed: 43 skills, 39 checklists, and 35 playbooks are present in the repo.
...
- Cross-model support: Confirmed: BosskuAI now includes a dedicated cross-model escalation skill so Claude, Codex, and Cursor can proactively bring in another model or tool surface when the current one is blocked, brittle, or mismatched for the task.
