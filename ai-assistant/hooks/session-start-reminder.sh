...
bossku_home="$(resolve_bossku_home)"

{
  echo "[BosskuAI] Auto-enforce — memory first, plan first, audit last:"
  echo "  1. READ   ${bossku_home}/ai-assistant/memory/active-continuation.md when active"
  echo "  2. QUERY  python3 ${bossku_home}/ai-assistant/scripts/auto_memory.py query '<task>' --limit 5"
  echo "  3. ROUTE  intent→cluster→skill via ${bossku_home}/AGENTS.md"
