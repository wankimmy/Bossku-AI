...
2. **Check patterns** — Does the new code follow established project patterns? Flag deviations with a clear rationale for why the pattern exists.
3. **Check edge cases** — What happens with null, empty, zero, very large, or malformed inputs? Are all async error paths handled?
4. **Check naming** — Are identifiers descriptive and consistent with the project's lexicon? Are functions doing exactly what their names say?
5. **Check tests** — Are new code paths covered? Are tests testing behavior (not implementation)? Are failure cases tested?
6. **Summarize** — Produce a structured review with severity-tagged findings. Give an overall verdict: Approve / Approve with suggestions / Request changes.
